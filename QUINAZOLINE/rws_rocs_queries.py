...
def run_benchmark(idx,dir_path):
    input_dict = read_input("rws_quinazoline_rocs_az.json","quinazoline_rocs.parquet",f"RUN_{idx:03d}")
    df_list = []
    ...
    pd.concat(df_list).to_csv(f"{dir_path}/rws_quinazoline_{idx:03d}.csv",index=False)        
