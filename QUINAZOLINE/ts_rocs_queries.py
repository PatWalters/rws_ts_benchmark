...
def run_benchmark(idx, dir_path):
    input_dict = read_input("ts_quinazoline_rocs.json","quinazoline_rocs.parquet",f"RUN_{idx:03d}")
    df_list = []
    ...
    pd.concat(df_list).to_csv(f"{dir_path}/ts_quinazoline_{idx:03d}.csv",index=False)
    

def main():
    dir_path = "ts_results_18500"
